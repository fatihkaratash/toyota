...
public class MainCoordinator implements Coordinator, PlatformCallback {
    ...
    private final Map<String, String> rateStatuses = new ConcurrentHashMap<>();
    private final Map<String, Boolean> platformConnections = new ConcurrentHashMap<>();
    ...
    public void onConnect(String platformName) {
        logger.info("{} platformu bağlandı", platformName);
        platformConnections.put(platformName, true);
        statusMessages.put(platformName, "Bağlantı kuruldu");
    }
    
    @Override
    public void onDisconnect(String platformName, String reason) {
        logger.info("{} platformu bağlantısı kesildi: {}", platformName, reason);
        platformConnections.put(platformName, false);
        statusMessages.put(platformName, "Bağlantı kesildi: " + reason);
    }
    
    @Override
    public void onRateAvailable(Rate rate) {
        if (rate == null) {
            logger.warn("Platform geri aramasından null kur alındı (onRateAvailable).");
            return;
        }
        
        logger.debug("{} için {} platformundan ilk kur alındı", 
                    rate.getSymbol(), rate.getPlatformName());
        
        // Store the rate status
        rateStatuses.put(rate.getPlatformName() + "_" + rate.getSymbol(), "AVAILABLE");
    ...
    public void onRateUpdate(Rate rate) {
        if (rate == null) {
            logger.warn("Platform geri aramasından null kur alındı (onRateUpdate).");
            return;
        }
        ...
        // Store the rate status
        rateStatuses.put(rate.getPlatformName() + "_" + rate.getSymbol(), "UPDATED");
    ...
    @Override
    public void onRateStatus(String symbol, String platformName, String status) {
        logger.info("{} platformundan {} sembolü için durum değişikliği: {}", 
                   platformName, symbol, status);
        
        // Store the rate status
        rateStatuses.put(platformName + "_" + symbol, status);
    }
    ...
    /**
     * Get the latest status for a specific rate
     * 
     * @param platformName Name of the platform
     * @param symbol The rate symbol
     * @return The latest rate status, or null if none exists
     */
    public String getRateStatus(String platformName, String symbol) {
        return rateStatuses.get(platformName + "_" + symbol);
    }
     ...
     */
    public boolean isPlatformConnected(String platformName) {
        Boolean connected = platformConnections.get(platformName);
