...
public class KafkaRateProducer implements RateProducer {
    ...
    public void sendRate(String topic, RateMessage message) {
        ...
        LoggingHelper.setCorrelationId("kafka-" + topic + "-" + message.getSymbol());
        ...
        try {
            ...
            future.whenComplete((result, ex) -> {
                if (ex == null) {
                    ...
                    LoggingHelper.logError(logger, "KafkaRateProducer", 
                            "Mesaj Gönderimi", "Mesaj=[" + message.getSymbol() + "] gönderilemedi", ex);
            ...
            getErrorCounter(topic).incrementAndGet();
            LoggingHelper.logError(logger, "KafkaRateProducer", 
                    "Mesaj Gönderimi", "Kafka konusu " + topic + " için mesaj gönderilirken istisna", e);
        } finally {
