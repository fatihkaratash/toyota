...
import com.toyota.mainapp.calculator.engine.JavaScriptEngine;
...
public class CalculationConfig {
    ...
    public JavaScriptEngine javaScriptEngine() {
        return new JavaScriptEngine(scriptsDirectory);
    }
    ...
    @Bean
    public List<CalculationStrategy> calculationStrategies() {
                    ...
                    try {
                        ...
                        switch (type.toLowerCase()) {
                            case "groovy":
                                ...
                                break;
                            case "javascript":
                                String jsScript = (String) strategyConfig.get("script");
                                if (jsScript != null) {
                                    @SuppressWarnings("unchecked")
                                    List<String> sourceSymbols = (List<String>) strategyConfig.get("sourceSymbols");
                                    JavaScriptEngine engine = javaScriptEngine(); // Get bean
                                    engine.registerScript(symbol, jsScript, sourceSymbols);
                                    calculationStrategies.add(engine.getStrategy(symbol));
                                    logger.info("{} için JavaScript hesaplama stratejisi kaydedildi", symbol);
                                }
                                break;
                            case "java":
